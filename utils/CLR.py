...
class CLR(object):
    ...
    def calc_lr(self, loss):
        ...
        self.running_loss = self.avg_beta * self.running_loss + (1-self.avg_beta) * loss
        smoothed_loss = self.running_loss / (1 - self.avg_beta**self.iteration)

        if math.isnan(smoothed_loss) or smoothed_loss > 4 * self.best_loss:
            return -1
        if smoothed_loss < self.best_loss and self.iteration > 1:
